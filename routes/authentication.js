...
module.exports = (router) => {
    ...
    router.post('/register', (req, res) => {
        if (!req.body.email) {
            res.json({ success:false, message: 'Debe ingresar un correo electronico'});
        } else {
            if (!req.body.username) {
                res.json({ success:false, message: 'Debe ingresar un nombre de usuario'});    
            } else {
                if (!req.body.password) {
                    res.json({ success:false, message: 'Debe ingresar una clave de acceso'});    
                } else {
                    let user = new User({
                    ...
                    user.save((err) => {
                        if (err) {
                            if (err.code === 11000){
                                res.json({
                                    success: false,
                                    message: 'Nombre de Usuario o E-mail ya existen'
                                });
                            } else {
                                if (err.errors){
                                    if (err.errors.email) {
